...
- **📡 RSS转ICS**：输入 RSS / Atom 订阅地址，抓取文章列表预填进可编辑表格（标题/日期/时间/地点/描述/链接/提醒），改好后导出 `.ics`；也可一键生成 **webcal / https 订阅链接**，让日历 App 长期自动刷新。预设少数派、阮一峰周刊、Hacker News、GitHub Releases；**兼容 RSSHub**（用默认 RSS 或 `.atom` 输出即可，`?format=json` 不支持）；支持设置默认时长（30 分 / 1 小时 / 2 小时 / 全天）、时区与 UID 前缀。
...
- **兼容 RSSHub**：RSSHub 生成的订阅源就是标准 **RSS 2.0**（默认）或 **Atom**（`.atom` / `?format=atom`），可直接粘进「订阅地址」框使用；**不支持** `?format=json`（JSON Feed，会明确报「不是 RSS / Atom」）。出于同样原因**没做 RSSHub 预设按钮**——公共实例常限流或不可达（实测 `rsshub.app` 超时、`rsshub.rssforever.com` 返回 503 占位页），点一下必然报错的预设不如让你直接粘自己的实例地址；建议自建或换成你所在网络可访问的实例。
